...
if calc_choice == "Grade Effect Calculator":
    ...
    current_grade_effect = st.number_input("📈 Your Current Grade (%)", min_value=0.0, max_value=100.0, step=0.1, key="effect_current")

    total_points_based = st.radio("Is this test graded on total points?", ["Yes", "No"], index=1)
    
    if total_points_based == "Yes":
        total_points = st.number_input("📊 Total Points Possible", min_value=1, step=1)
        final_worth = st.number_input("🏆 Final Worth Points", min_value=1, step=1)
        test_score = st.number_input("✅ Test/Quiz Score (Points Earned)", min_value=0.0, step=0.1)
        ...
        test_weight = st.slider("📝 Test/Quiz Weight (%)", 1, 50, 10)
        test_score = st.number_input("✅ Test/Quiz Score (%)", min_value=0.0, max_value=100.0, step=0.1)
    ...
    desired_grade_input = st.text_input("### Step 5: Enter Your Desired Grade (Letter or %)")
...
st.text("Version 1.4 - Added Total Points Option in Grade Effect Calculator!")
